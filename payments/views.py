...
class CreateCheckoutView(LoginRequiredMixin, View):
    def get(self, request: HttpRequest, plan: str) -> HttpResponse:
        ...
        if provider == "pagarme":
            ...
            checkout_url = (
                payment_link.get("url")
                or payment_link.get("checkout_url")
                or payment_link.get("payment_url")
                or payment_link.get("link")
            )
            if not checkout_url:
                checkout_id = payment_link.get("id")
                if checkout_id:
                    checkout_url = f"{settings.PAGARME_PAYMENT_LINK_URL_BASE}/{checkout_id}"
            if not checkout_url:
                logger.warning(
                    "[pagarme] Link sem URL (keys=%s).",
                    sorted(payment_link.keys()),
                )
            ...
            Payment.objects.create(
                ...
                mp_payment_id=str(payment_link.get("id", "")),
                external_reference=external_reference,
                raw_payload=payment_link,
            )
